...
Calendar_Import={
	...
	Dialog:{
		open: function(filename){
			...
			$('#calendar_import').load(OC.generateUrl('apps/calendar/getimportdialogtplcalendar'), {filename:Calendar_Import.Store.file, path:Calendar_Import.Store.path},function(){
		...
		update: function(){
			...
			$.post(OC.generateUrl('apps/calendar/importeventscalendar'), {progresskey: Calendar_Import.Store.progresskey, getprogress: true}, function(data){
 				if(data.status == 'success'){
					...
					if(data.percent < 100 ){
						window.setTimeout('Calendar_Import.Dialog.update()', 100);
					}else{
