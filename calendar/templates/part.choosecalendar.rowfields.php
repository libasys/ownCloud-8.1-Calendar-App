...
<?php	
if($_['calendar']['id'] != 'birthday_'.OCP\USER::getUser()){?>	
  <a href="<?php print_unescaped(\OC::$server->getURLGenerator()->linkToRoute('calendar.export.exportEvents') . '?calid=' . $_['calendar']['id']) ?>" title="<?php p($l->t('Download')) ?>" class="permanent action"><i class="ioc ioc-download"></i></a>
<?php } ?>
</td>
<td width="20px">
