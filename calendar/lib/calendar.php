...
class Calendar{
	...
	public static function allCalendars($uid, $active=false, $bSubscribe = true) {
		...
		$calendars = array_merge($calendars, \OCP\Share::getItemsSharedWith('calendar', \OCA\Calendar\Share\Backend\Calendar::FORMAT_CALENDAR));
   ...
   public static function checkGroupRightsForPrincipal($uid){
   	   ...
   	   $isEnabled=$appConfig->getValue('calendar','enabled');
	   $bEnabled=false;
	   if ($isEnabled === 'yes') {
	   	   $bEnabled=true;
	   } 
	...
	public static function addCalendar($userid,$name,$components='VEVENT,VTODO,VJOURNAL',$timezone=null,$order=0,$color="#C2F9FC",$issubscribe=false,$externuri='',$lastmodified=0 ) {
		...
		$stmt = \OCP\DB::prepare( 'INSERT INTO `*PREFIX*clndr_calendars` (`userid`,`displayname`,`uri`,`ctag`,`calendarorder`,`calendarcolor`,`timezone`,`components`,`issubscribe`,`externuri`,`lastmodifieddate`) VALUES(?,?,?,?,?,?,?,?,?,?,?)' );
	...
	public static function addDefaultCalendars($userid = null) {
		...
		\OCP\Config::setUserValue($userid, 'calendar', 'choosencalendar',$id);
	...
	public static function addCalendarFromDAVData($principaluri,$uri,$name,$components,$timezone,$order,$color,$transparent) {
		...
		$stmt = \OCP\DB::prepare( 'INSERT INTO `*PREFIX*clndr_calendars` (`userid`,`displayname`,`uri`,`ctag`,`calendarorder`,`calendarcolor`,`timezone`,`components`,`transparent`) VALUES(?,?,?,?,?,?,?,?,?)' );
	...
	public static function editCalendar($id,$name=null,$components=null,$timezone=null,$order=null,$color=null, $transparent = null) {
		...
		$stmt = \OCP\DB::prepare( 'UPDATE `*PREFIX*clndr_calendars` SET `displayname`=?,`calendarorder`=?,`calendarcolor`=?,`timezone`=?,`components`=?,`ctag`=`ctag`+1, `transparent`=?  WHERE `id`=?' );
	...
	public static function getEventSourceInfo($calendar, $isPublic = false) {
		...
		return array(
			'url' => \OC::$server->getURLGenerator()->linkToRoute('calendar.event.getEvents').'?calendar_id='.$calendar['id'],
